...
/**
 * Filter chips + animated project list. Receives its projects from the
 * server component, which generates them from the live GitHub repo list.
 */
export function ProjectsGallery({ projects }: { projects: Project[] }) {
  const [filter, setFilter] = useState<FilterId>("all");

  // Only show a filter chip if some project actually carries that tag.
  const filters = useMemo(() => {
    const tags = new Set<ProjectTag>(projects.flatMap((p) => p.tags));
    return PROJECT_FILTERS.filter(
      (f) => f.id === "all" || tags.has(f.id as ProjectTag),
    );
  }, [projects]);
